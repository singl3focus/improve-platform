...
import type { TodayTask, TodayMaterial } from "@features/today/types";
    ...
    <div className={`today-task${task.isCompleted ? " today-task-done" : ""}`}>
      ...
      >
        {task.isCompleted ? (
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
            <rect x="1" y="1" width="18" height="18" rx="4" fill="var(--accent)" stroke="var(--accent)" strokeWidth="2" />
      ...
      <div className="today-task-content">
        ...
        <div className="today-task-meta">
          {task.topicTitle && <span className="today-task-topic">{task.topicTitle}</span>}
          {task.deadline && (
            <span className={`today-task-deadline${isOverdue(task.deadline) && !task.isCompleted ? " today-task-deadline-overdue" : ""}`}>
              {task.deadline}
            </span>
          )}
        </div>
    ...
    <div className="today-material">
      ...
      <div className="today-material-progress">
        <div className="today-material-bar">
          <div
            className="today-material-bar-fill"
...
const RU_TODAY_COPY: TodayCopy = {
  ...
  focusTasks: "Фокус-задачи",
  noTasks: "На сегодня задач нет. Задачи появятся автоматически или добавьте вручную.",
  currentMaterial: "Текущий материал",
  reflection: "Микро-рефлексия",
  reflectionPlaceholder: "Что я узнал сегодня?",
  reflectionSave: "Сохранить",
  reflectionSaving: "Сохраняю...",
  reflectionSaved: "Сохранено",
  loading: "Загрузка...",
...
const EN_TODAY_COPY: TodayCopy = {
  ...
  noTasks: "No tasks for today. Tasks will auto-populate or you can add them manually.",
  currentMaterial: "Current material",
  reflection: "Micro-reflection",
  reflectionPlaceholder: "What did I learn today?",
  reflectionSave: "Save",
...
export function TodayView() {
  ...
  const lang = appCopy.navigation.brand ? (appCopy.navigation.ariaPrimary === "Основная навигация" ? "ru" : "en") : "en";
  ...
  const hasSavedReflection = today.reflection !== null && today.reflection !== "";
  ...
  return (
    <div className="today-page">
      ...
      <section className="today-section">
        ...
        <div className={`today-reflection-editor${hasSavedReflection && !draftChanged ? " today-reflection-editor-collapsed" : ""}`}>
          <textarea
            ref={textareaRef}
